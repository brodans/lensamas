...
export class VideoRTC extends HTMLElement {
    ...
    play() {
        this.video.play().catch(() => {
            // Autoplay WebRTC often requires a muted retry, including when
            // CameraPlayer has already muted the element before negotiation.
            this.video.muted = true;
            this.video.play().catch(() => undefined);
        });
    }

    /** Informasi frame pertama agar UI tidak menampilkan LIVE saat video masih hitam. */
    emitFrame(mode) {
        this.dispatchEvent(new CustomEvent('videortc-frame', {
            detail: {mode},
        }));
    }
    ...
    onmse() {
        ...
        this.onmessage['mse'] = msg => {
            ...
            sb.addEventListener('updateend', () => {
                ...
                if (!sentFrame && sb.buffered && sb.buffered.length) {
                    sentFrame = true;
                    this.emitFrame('mse');
                }
        ...
        this.onmessage['webrtc'] = msg => {
            switch (msg.type) {
                case 'webrtc/candidate':
                    ...
                    pc.addIceCandidate({candidate: msg.value, sdpMid: '0'}).catch(() => undefined);
                    break;
                case 'webrtc/answer':
                    pc.setRemoteDescription({type: 'answer', sdp: msg.value}).catch(() => undefined);
                    break;
                case 'error':
    ...
    async createOffer(pc) {
        try {
            ...
            // getUserMedia opsional; stream receive tetap dapat berjalan.
        }
